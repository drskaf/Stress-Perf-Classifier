...
niftiFiles, videoSingles, indicesSingles, videoStacks, indicesStacks = utils.load_perfusion_data(args["directory"])
...
"""EXTRACT PEAK LV from single dicome files"""
for nifti, videoraw, i in zip(niftiFiles, videoSingles, indicesSingles):
        test = {}
        keys = range(len(nifti[:,]))
        # Define series with AIF frames
        for k in keys:
            # Calculate image sharpness
            img = utils.centre_crop(nifti[k])
            gy, gx = np.gradient(img)
            gnorm = np.sqrt(gx ** 2, gy ** 2)
            sharp = np.average(gnorm)
            test[k] = sharp
        ...
        if aif_sharp < nonaif_sharp // 2:
            ...
            aif_max_key = [key for key, value in aif_5max.items() if value == np.max(aif_max_value)]
            aif_l = aif_max_key.pop()
            for img in nifti[aif_l:aif_l + 5]:
                plt.imshow(img, cmap='gray')
                plt.show()
